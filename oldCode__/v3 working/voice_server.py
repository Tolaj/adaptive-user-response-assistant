...
for path in [
    ...
    os.path.expandvars(
        r"%LOCALAPPDATA%\Microsoft\WinGet\Packages\Gyan.FFmpeg_Microsoft.Winget.Source_8wekyb3d8bbwe\ffmpeg-7.1-essentials_build\bin\ffmpeg.exe"
    ),
]:
...
def find_model_file(model_key: str) -> Path:
    ...
    gguf_files = [
        f for f in model_folder.glob("*.gguf") if not f.name.startswith("mmproj")
    ]
    if not gguf_files:
...
def generate_llm_response(
    messages, max_tokens=512, temperature=0.7, top_p=0.9, stream=False
):
    ...
    try:
        with lock:
            output = generate_llm_response(
                messages, max_tokens=VOICE_MAX_TOKENS, temperature=VOICE_TEMPERATURE
            )
