...
def send_voice(audio_data):
    ...
    audio_16k = resampy.resample(audio_mono.astype(np.float32), SAMPLE_RATE, WHISPER_RATE)
    tmp_path = os.path.join(tempfile.gettempdir(), f"voice_{int(time.time())}.wav")
    sf.write(tmp_path, audio_16k, WHISPER_RATE)
    print(f"   Audio: {len(audio_16k)/WHISPER_RATE:.1f}s at 16kHz")
