...
def load_audio_file(path: str) -> np.ndarray:
    """
    Load any audio file (wav, mp3, webm) as a 16kHz float32 numpy array.
    Tries soundfile first, falls back to converting via scipy for webm.
    """
    ...
    try:
        ...
        # soundfile can't read webm — try reading raw and resampling
        # For webm from browser, we need a different approach
        raise RuntimeError(
            f"Cannot read audio file: {path}\n"
            "soundfile cannot decode webm. Install ffmpeg or use WAV format."
        )
...
def voice_chat():
    ...
    try:
        ...
        audio_data = load_audio_file(tmp_path)
        print(f"   Audio shape: {audio_data.shape}, max: {audio_data.max():.3f}, sample_rate sent: 16000")
        result = whisper.transcribe(WHISPER_MODEL, audio_data, language="en", verbose=True)
